...
| 能力 | 落地 | 权威 |
|---|---|---|
| 远程智能体静态接入，获取其可调用能力 | §4.1 | `version-scope/FEAT-004-task-driven-remote-agent-communication.md:31` |
| 单个下游调用代理，结果返回后恢复父任务 | §4.3 | 同文 `:32` |
| 批量委托消费，保持每个委托的独立身份 | §4.4 | 同文 `:33` |
| 受控并发预算内并行发起 | §4.5 | 同文 `:34` |
| all-settled 汇聚：全部结果性终态才回填 | §4.6 | 同文 `:35` |
| 按委托关联键关联结果，不得按顺序或名称猜测 | §4.6 | 同文 `:36` |
| 父任务可观察下游调用的关联状态 | §4.7 | 同文 `:37` |
| 远端标识内部关联，用于续轮路由与诊断 | §5.2 | 同文 `:38` |
| 结果与错误并存，作结构化结果回填 | §4.6 | 同文 `:39` |
| 父任务单次恢复 | §4.6 | 同文 `:40` |
| 超时治理，超时项作结果性失败参与汇聚 | §4.5 | 同文 `:41` |

**另有一条能力经跨特性划分归入本特性**，不在上表的 11 条之内：

| 能力 | 落地 | 权威 |
|---|---|---|
| 异步回调回灌：远端主动投递终态时的认领与分派 | §4.8 | `version-scope/FEAT-001-standardized-agent-service-entrypoint.md` 的非目标表（outbound 远程 Agent 编排一行）（结果回灌由远程编排特性承接）、同文 `:85`（场景要求「附着回本地上下文并恢复等待中的执行链或 Task」）|
...
**异步回调的分派归编排层，入站适配层只调用不实现**（§4.8）。接收入口位于入站适配层，它经编排入口把载荷交进来；编排层再委托批次协调件认领。适配层**不得**直接调批次协调件——那会让入站适配层依赖出站适配层，属同层横向依赖，§3.3 已列为禁止。
...
本版遵从该约束的**实质**：`remote_batch.py` 内的批次与成员状态是该模块的内部结构，不进 `domain/`、不成为公共端口类型（§3.3 禁止项第三条）。
...
**§4.8 新增的调用链方向同样由外向内**：入站适配层→编排层→批次组件。**显式禁止一条**：入站适配层不得直接调批次组件——那是入站适配层依赖出站适配层的同层横向依赖（§3.1、§3.3）。
