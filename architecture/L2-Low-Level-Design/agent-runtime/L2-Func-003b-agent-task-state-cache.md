...
| 能力 | 级别 | 事实要求 | 状态  权威出处 |
|---|---|---|------|
| 接入形态配置 | MUST | 配置可指定单机或集群；未配置时按单机处理，既有单机配置继续可用 | 已实现  `FEAT-003:38`（MUST） |
| 单机接入 | MUST | 配置为单机时能完成读写 | 已实现  `FEAT-003:39`（MUST） |
| 集群接入 | MUST | 配置为集群时以**同一端口**完成读写；路由与重定向由实现承接，调用方不感知 | 已实现  `FEAT-003:40`（MUST） |
| 集成方封装扩展点 | MUST | 注入自定义实现即接管全部 Redis 操作，业务代码不改 | 已实现  `FEAT-003:41`（MUST） |
| 统一操作接口 | MUST | 三种接入形态对上层暴露完全相同的方法面 | 已实现  `FEAT-003:42`（MUST） |
| 启动策略日志 | MUST | 启动日志体现当前接入形态、实现类型与非敏感连接摘要 | 已实现  `FEAT-003:43`（MUST） |
| 口令日志脱敏 | MUST | 口令、密文、令牌不出现在任何日志与异常信息中 | 已实现  `FEAT-003:44`（MUST） |
| 配置切换 | MUST | 单机与集群间切换只改配置；默认与自定义实现间切换只换注入，均不改业务代码 | 已实现  `FEAT-003:45`（MUST） |
| 状态缓存过期时间 | SHOULD | Task 快照与框架状态使用统一的可配置过期时间，默认 604800 秒 | 已实现  `FEAT-003:46`（SHOULD） |
| 复用同一数据源 | SHOULD | Task 存储与框架状态持久化复用同一接入抽象 | 已实现  `FEAT-003:47`（SHOULD） |
| 集成方模式内部不可复现说明 | MUST | 文档明确：集成方自封装组件不外传，内部只验证可适配能力与默认形态 | 已实现（§12.3）  `FEAT-003:48`（MUST） |
| 配置错误可诊断 | SHOULD | 接入形态未知、连接配置缺失时给出明确错误，不静默退回 | 已实现  `FEAT-003:49`（SHOULD） |
| 资源生命周期 | SHOULD | 连接资源随应用生命周期初始化与释放 | **部分实现**（见 §13）  `FEAT-003:50`（SHOULD） |
| Todolist 自治持久化 | MUST | 框架侧经注入的存储对象自治读写，runtime 只提供连接、不解释内容 | **部分实现**（上游未实现，见 §13）  `FEAT-003:51`（MUST） |
