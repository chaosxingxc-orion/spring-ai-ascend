...
| Feat-Func-022 | [Custom REST API 到 Agent Runtime 执行入口适配 SPI](Feat-Func-022-custom-rest-api-to-a2a-jsonrpc-adaptation-spi.md) | 可配置 Custom REST URL、Java 协议转换 SPI、同步 JSON/SSE 响应包装。 | 设计已接受、代码待落地于 solution 扩展仓；请求经 `RequestHandler` 创建或恢复正式 A2A Task。 |
