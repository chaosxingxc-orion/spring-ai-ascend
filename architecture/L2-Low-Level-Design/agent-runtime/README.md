...
| Feat-Func-009 | [【调用端侧工具响应】新增支持带有端侧工具的请求](Feat-Func-009-调用端侧工具响应-新增支持带有端侧工具的请求.md) | JSON-RPC A2A 端侧工具能力视图、任务级动态工具可见性、`INPUT_REQUIRED` 调用移交和客户端结果回灌。 | runtime 持有 Task 状态，solution runtime extension 以请求级 rail 适配 ReActAgent/受限配置的 DeepAgent；Core/Runtime 生产代码不修改。 |
