...
| Feat-Func-022 | [Custom REST API 到 Agent Runtime 执行入口适配 SPI](Feat-Func-022-【自定义智能体服务入口】自定义%20RestAPI%20到%20runtime内部编排层协议适配转换%20SPI.md) | 可配置 Custom REST URL、Java 协议转换 SPI、同步 JSON/SSE 响应包装。 | 独立的 Custom REST 非 Task facade 特性；与 Feat-Func-001 关联并复用 `ServeOrchestrator`，不扩展 A2A Task 表面。 |
