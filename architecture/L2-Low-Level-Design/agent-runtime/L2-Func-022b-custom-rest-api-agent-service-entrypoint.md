...
| 能力 | 级别 | 事实要求 | 状态 | 权威出处 |
|---|---|---|---|---|
| 自定义 REST 服务入口 | MUST | 启用一个自定义 REST 服务化入口，调用方经该路径发起执行。**本版路径形状写死在适配件内、不可配置**——可配置能力见 §2.2 显式排除 | 已实现 | `FEAT-022:37`（MUST） |
| 单 path | MUST | 只有**一个业务路径模式** `/v1/{通道路径}`（通道路径含斜杠、按整体匹配）；同前缀下另有其取消子路径，归「取消」那条（`FEAT-022:46`）。不支持同一入口挂多个兼容业务路径 | 已实现 | `FEAT-022:38`（MUST） |
| 同步消息调用 | MUST | 一次 HTTP 请求对应一次消息提交，以 JSON 响应返回结果或错误投影 | 已实现 | `FEAT-022:42`（MUST） |
| 流式消息调用 | MUST | 以 SSE 返回执行过程，帧形态与存量逐字一致 | 已实现 | `FEAT-022:43`（MUST） |
| 请求映射 | MUST | HTTP 请求体到标准调用上下文的映射：本版按**固定映射**承接（§3 承接表 S2），**用户可扩展的映射规则不交付**——见 §2.2 显式排除 | **部分承接**（固定映射；可扩展接口见 §2.2） | `FEAT-022:40`（MUST，部分承接）|
| 响应投影 | MUST | 24 种事件类型各自投影为对外信封，字段集与键序与存量一致 | 已实现 | `FEAT-022:41`（MUST） |
| 错误投影 | MUST | 执行前的内容类型／JSON 解析错误、执行中的错误，各自的信封与状态码与存量一致 | 已实现 | `FEAT-022:41`（MUST） |
| 取消 | MUST | 取消端点映射到标准取消语义，响应体与存量一致 | 已实现 | `FEAT-022:46`（SHOULD） |
| 语义归一 | MUST | 执行、Task 生命周期、状态、错误、超时、取消边界归一到标准入口，不定义独立状态机 | 已实现 | `FEAT-022:44`（MUST） |
| 单智能体边界 | MUST | 路径中的业务字段只作上下文，不作同实例内多智能体路由依据 | 已实现 | `FEAT-022:39`（MUST） |
