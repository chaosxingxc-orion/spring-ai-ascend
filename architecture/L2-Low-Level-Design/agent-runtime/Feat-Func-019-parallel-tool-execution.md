...
- **边界**：同一 conversation 的不同父 Task 必须顺序进入 Handler/Core；并发进入的行为不属于本特性承诺或验收范围。parentTaskId 分键只用于受支持顺序下的路由隔离、陈旧请求拒绝和精确取消，不改变 Core session 仍按 conversationId 保存的事实。
...
- **必须**：成员进入 `QUEUED/RUNNING/INPUT_REQUIRED/COMPLETED/FAILED/TIMED_OUT/CANCELED` 时，通过父 Task Artifact 投影脱敏状态；`toolCallId` 由 coordinator 从成员上下文注入，不信任远端载荷。
...
| runtime 有界 dispatcher | 控制活动调用数量、FIFO 排队、队列容量、排队超时和 queued 取消 | 创建第二套远端调用线程池 |
...
| 异步有界 dispatcher | 不新增线程池，以活动计数和 FIFO 队列决定何时调用现有 A2A Client | 可取消 queued member；有明确容量和排队语义 | coordinator 内增加少量状态机 | 采用 |
| 阻塞式 Semaphore | 提交线程调用 `acquire()` 后再发起远端调用 | 实现简单 | queued 取消、排队超时和状态投影困难；可能长期阻塞 orchestrator 线程 | 拒绝 |
...
- 单中断可继续使用单成员 map 载体，但必须包含真实 `toolCallId`；orchestrator 将其归一化为大小为 1 的批次。缺失时返回 `CORE_INTERRUPT_CORRELATION_MISSING`，不生成伪 ID。
...
`A2ARemoteAgentClient` 统一暴露 coordinator 使用的结构化流式调用，不保留只返回字符串的旧调用入口，也不新增独立 DTO 文件：

```java
CompletableFuture<RemoteCallOutcome> callStreamingOutcome(
    RemoteCall call,
    QueryStreamObserver progressObserver,
    Consumer<String> remoteTaskIdObserver)
```

`remoteTaskIdObserver` 只在远端状态事件携带 Task ID 时通知 coordinator，用于尽早保存取消路由；中间远端状态不再复制成第二套内部 DTO。`RemoteCallOutcome` 作为 `A2ARemoteAgentClient` 的内部 record 定义：

| 类型 | 字段 | 语义 |
|---|---|---|
| `RemoteCallOutcome` | `remoteTaskId`, `remoteState`, `resultCategory`, `result`, `inputPrompt` | Future 完成时返回结果性状态；不得把不同终态都折叠为空字符串。 |

状态映射至少区分 `COMPLETED`、`INPUT_REQUIRED`、`REJECTED`、`CANCELED`、远端业务失败、限流、协议错误和连接不可达；超时仍由 endpoint timeout 转为 `REMOTE_TIMEOUT`。`INPUT_REQUIRED` 和其他结果均通过 `RemoteCallOutcome` 返回，由 coordinator 统一处理，不再通过旧异常或字符串接口表达。
...
续轮携带该成员的 `remoteTaskId`，并用同一组 `parentConversationId/batchId/toolCallId` 重新得到完全相同的 `remoteContextId`。`batchId` 用于隔离父会话中的不同执行轮次，不能只拼接可能跨轮复用的 `toolCallId`。单成员批次继续直接使用父 `conversationId`，保持现有远端会话兼容。该值可由 shadow Task 的 contextId、`_remote_batch.batchId` 和 member `toolCallId` 确定性重建，不新增 `remoteContextId` 持久化字段。

每个成员的 `RemoteCall.metadata` 从已过滤的请求 metadata 创建独立副本，并由 coordinator 注入该成员的 `runtime.idempotencyKey`。禁止把父 Task `_interrupt`、`runtime.parentTaskId`、`runtime.remoteToolInputs`、`runtime.remoteToolResults` 或其他成员的输入/结果发送给下游 Agent；`A2ARemoteAgentClient` 仅通过现有 `MessageSendParams.metadata` 传递这个成员级副本。
...
- `members[].remoteTaskId/inputPrompt` 保存 B/C 的远端续轮路由；流式远端调用是 Runtime 的统一执行方式，不需要在中断或 shadow 中保存模式字段。多成员的远端 `contextId` 由 shadow contextId、`batchId` 和 `toolCallId` 确定性重建，不重复保存。
...
| 活动批次索引 | coordinator 内存中的 `parentTaskId -> Batch`、`conversationId -> parentTaskId` | 仅批次运行期间 | 前者用于 A2A 父 Task 精确取消，后者用于受支持顺序场景下的 lifecycle interrupt/reset；等待输入后以 parentTaskId shadow 为准，不依赖内存索引。该索引不承担 conversation 并发准入控制。 |
| shadow Task 标识 | `TaskStore`，key 为 `shadow:<agentId>:<parentTaskId>` | 随 TaskStore 实现 | 复用现有 TaskStore，只按父 Task ID 定位 `_remote_batch`。 |
| 完成结果 A/B/C | shadow `Task.metadata["_remote_batch"].members[].result` | `WAITING_INPUT` 和 `READY_TO_RESUME` 期间持久化 | 先完成的结果不回灌、不重跑；Core 正常返回后随 shadow Task 删除，Core 失败时保留供幂等重试。 |
...
| 出站成员 metadata | 每个 `RemoteCall.metadata` 的独立副本 | 不持久化 | 保留允许透传的非控制 metadata，只注入当前成员的 `runtime.idempotencyKey`；不携带 `_interrupt`、批次输入、批次结果或父 Task 内部路由字段。 |
...
| 成员状态投影 | 真实父 Task Artifact 的 TextPart metadata `"_remote_invocation"` | 随父 Task Artifact | 每次成员状态变化时投影 `batchId/toolCallId/sequence/target/phase/resultCategory/latencyMs`；不作为 coordinator 恢复依据。 |
...
1. `JiuwenCoreAgentHandler` 收齐 core 的全部中断后才返回一个现有 interrupt data batch envelope，并拒绝混合 interrupt kind；全部为本地 `ask_user` 的批次仍作为普通 interrupt 原样返回，不进入远端 coordinator。
2. orchestrator 仅把全部 item 均为 `a2a_delegate` 的批次交给 coordinator；coordinator 校验 `toolCallId` 唯一、远端 Agent 可解析、当前不存在活动批次。
3. coordinator 把成员提交给内部有界 dispatcher；最多 `max-concurrency` 个成员进入 `RUNNING` 并调用 `A2ARemoteAgentClient.callStreamingOutcome()`，通过状态 observer 尽早保存远端 Task ID，通过结构化 outcome 获取终态类别；其余成员按原顺序进入 FIFO `QUEUED`，不新建 runtime 线程池。
...
8. 全部成员为 `COMPLETED/FAILED/TIMED_OUT` 时先保存 `READY_TO_RESUME` shadow，再形成完整结果 map 恢复 core；Core 正常返回或流式正常完成后删除 shadow，Core 失败时保留快照供重试。
...
“core resume 成功”只表示 `Runner.runAgent/runAgentStreaming` 正常完成本次调用，不取决于返回内容是最终答案还是新一批中断。调用 Core 期间旧 shadow 必须保留；Runner 正常返回后，orchestrator 必须先删除旧 shadow，再解析和处理返回内容，避免新旧批次共存。若 core resume 调用抛错，保留 `READY_TO_RESUME` shadow 快照，不删除 A/B/C 结果；后续只能以同一 `batchId` 做幂等重试。
...
| 远端业务进度正文 | 默认不实时输出 | 可随成员 Artifact 交错输出 |
| 批次屏障 | 相同 | 相同 |
...
- A2A 父 Task 的 `CancelTask` 必须按 `parentTaskId` 精确取消，不能仅按 conversationId 处理。`A2AAgentExecutor.activeCancellations` 的注册、查询和删除键统一从 contextId 改为 parentTaskId；`cancel()` 在 A2A orchestrator 场景调用其内部 `cancelParentTask(parentTaskId)`，其他 orchestrator 继续回退现有 `cancelActive(conversationId)`，不修改 `ServeOrchestrator` 公共 SPI。
- coordinator 对运行批次先查 `parentTaskId -> Batch` 活动索引；对已经进入 `INPUT_REQUIRED`、当前无在途调用的批次，按 `shadow:<agentId>:<parentTaskId>` 读取 `_remote_batch`。命中后标记批次 `CANCELED`，从 FIFO 移除 `QUEUED` 成员，并对已获得 `remoteTaskId` 的 `RUNNING/INPUT_REQUIRED` 成员 best-effort 发送 A2A CancelTask。
- 现有 `ActiveStreamRegistry` 继续只负责 conversation 级流停止和 lifecycle interrupt/reset，不增加 toolCallId 级能力。lifecycle 按 conversationId 取消时，orchestrator 通过内存 conversation 索引取消当前运行父 Task，并通过 TaskStore 的 conversation 查询定位后再按 Task ID 读取等待输入 shadow；父 Task `CancelTask` 只取消目标 parentTaskId，其他或陈旧 parentTaskId 不得命中当前批次。
- 每成员沿用对应远端 Agent 的 endpoint timeout；超时成员形成 `REMOTE_TIMEOUT` 工具结果，已取得 `remoteTaskId` 时 best-effort 发送 CancelTask，不无限阻塞批次。
- dispatcher 使用 `activeCount + FIFO Deque<PendingInvocation>`，不阻塞式获取 Semaphore，也不新增线程池。endpoint timeout 从真正发起远端调用开始计算，`queue-timeout` 从入队开始计算。
- dispatcher 随单例 coordinator 在 Runtime 实例内共享；`max-concurrency` 约束所有父 Task 的远端调用总量，不为每个批次分别创建配额。FIFO 只保证按全局提交顺序推进，不承诺父 Task 之间的公平份额；当前版本不增加 per-parent 配额。取消只移除对应父 Task 的 queued member。
...
└── agent-service-app/
    └── .../
        ...
        │   └── client/A2ARemoteAgentClient.java   # 修改：暴露远端 Task 状态和结构化 outcome
        ├── config/A2AProperties.java              # 修改：有界调度配置
...
| `A2AProtocolAdapter` | 把全部 TextPart 无条件拼成一个字符串，未把父 taskId 放入 ServeRequest，也未隔离外部伪造的内部控制 metadata | 普通消息保持拼接；目标续轮按 `toolCallId` 分组一次；先删除四个保留的 `runtime.*` 控制键，再写入内部 `runtime.parentTaskId/runtime.remoteToolInputs`。 |
...
| `A2AAgentExecutor` | 流式路径可逐 chunk 写 Artifact，非流式 `executeQuery()` 只处理最终 `QueryResponse`；取消标记按 contextId 保存且只传 conversationId | 继续作为父 Task 单 writer；非流式用一个 observer 消费 `streamQuery()`，实时写成员投影、聚合普通 chunk，并且只收尾一次；`activeCancellations` 改按 parentTaskId 注册/查询/删除，A2A CancelTask 把 parentTaskId 传给 A2A orchestrator 的内部精确取消方法。 |
...
在现有 A2A 配置下新增 Runtime 远端调用有界调度配置，并继续使用每个远端 Agent 的 timeout 控制实际调用超时：
...
- endpoint `timeout-seconds` 从实际调用开始计算，不包含排队时间。

core `730` 源码中的 `openjiuwen.executor.tool-call.*` 只控制 core ToolCall/Rail 线程池，与 Runtime 远端调用预算互不替代。Runtime 有界 dispatcher 只决定何时调用现有 A2A Client，不创建第二套执行线程池。
...
- 以 parentTaskId 查找内存活动批次或等待输入 shadow，不使用成员顺序或 active member。
- 未开始成员从全局 FIFO 移除，不再获得调用配额。
- 已有 remoteTaskId 的成员 best-effort CancelTask。
- 投影未完成成员的 `CANCELED` 状态，清理活动索引和已有 shadow；取消状态不新增持久化快照。
- 禁止再恢复 core；其他或陈旧 parentTaskId 的取消请求不能命中当前批次。同一 conversation 的下一父 Task 只能在当前父 Task 结束后启动。
...
| 成员超时 | 超过 endpoint `timeout-seconds` | best-effort cancel，成员 `TIMED_OUT` | 工具结果 `REMOTE_TIMEOUT` |
| 远端拒绝 | 远端 Task 进入 REJECTED | 成员 `FAILED`，保留拒绝类别 | 工具结果 `REMOTE_REJECTED` |
| 远端自行取消 | 非父 Task 取消触发的远端 CANCELED | 成员 `FAILED`，其他成员继续 | 工具结果 `REMOTE_CANCELED` |
...
稳定 `resultCategory` 至少包括：`REMOTE_REJECTED`、`REMOTE_CANCELED`、`REMOTE_TIMEOUT`、`REMOTE_RATE_LIMITED`、`REMOTE_PROTOCOL_ERROR`、`REMOTE_UNAVAILABLE`、`REMOTE_BUSINESS_FAILURE`、`REMOTE_OVERLOADED`。成员状态保持有限枚举，具体失败原因通过 `resultCategory` 和结构化工具结果表达。
...
不定义外层 kind 或 discriminator。adapter/orchestrator 使用结构化规则识别：`items` 是列表时按批次处理；单个 map 只有在包含真实 `toolCallId` 时才归一化为大小为 1 的批次。本地 `ask_user` 中断继续原样返回；`items` 为空、类型错误、远端成员缺少 `toolCallId` 或既有 `context._interrupt_kind` 时按协议错误拒绝。禁止把 list 截断为最后一个元素。
...
```text
runtime.idempotencyKey = <parentTaskId>:<batchId>:<toolCallId>
```

该键标识一个批次内的逻辑成员，同一成员的受控重试必须保持相同键；当前版本不定义自动重试策略，也不引入 `attempt` 状态。未来若增加自动重试，`attempt` 只能作为诊断计数，不能改变同一逻辑成员的幂等键。该键不能接受外部 A2A metadata 的值；coordinator 生成后写入过滤后的成员级 `RemoteCall.metadata`，由 `A2ARemoteAgentClient` 放入下游 `MessageSendParams.metadata`。runtime 对同一活动批次的重复执行先查询 shadow `_remote_batch`，已完成成员不重发。若进程在远端已接收请求但尚未保存 remoteTaskId 时崩溃，仍存在重复窗口；远端有副作用的 Agent 必须消费幂等键或自行提供重复保护。
...
| 内部 metadata 防伪造 | 外部 params metadata 中伪造的 `runtime.parentTaskId`、`runtime.remoteToolInputs`、`runtime.remoteToolResults`、`runtime.idempotencyKey` 均被删除；前两项由 adapter 重建，后两项只能由 coordinator 生成。 |
| 出站成员 metadata 隔离 | A/B/C 的 RemoteCall 各自只携带允许透传的 metadata 和自己的 `runtime.idempotencyKey`，不包含 `_interrupt`、父 Task 内部路由、批次输入/结果或其他成员数据。 |
...
| 全本地多中断 | 同轮多个 item 均为本地 `ask_user` 时原样返回普通 interrupt，不创建远端批次。 |
...
| queued 取消 | 父 Task 取消会移除未启动成员，且不会在释放配额后补发远端调用。 |
...
| 成员失败隔离 | B 失败不取消 A/C。 |
| 远端结果分类 | REJECTED/CANCELED/限流/协议错误/业务失败分别形成稳定 `resultCategory`。 |
| 超时 | 超时成员产生错误结果，批次不永久等待。 |
| 取消 | 未完成成员和远端 Task 被取消，late result 不回灌。 |
| 父 Task 精确取消 | `activeCancellations` 按 parentTaskId 注册、查询和删除；等待输入批次可由 parentTaskId shadow 取消，其他或陈旧 parentTaskId 不命中当前批次。 |
| client 启动窗口取消 | 父 Task 在 client 返回 Future 前取消时，Future 发布后立即取消并释放并发配额，不形成远端泄漏。 |
...
| READY 快照 | 全批终态后先保存 `READY_TO_RESUME`；Core 正常完成后删除，Core 报错时保留并可按原父 Task 幂等恢复。 |
...
12. 批次成员数超过并发预算时验证有限并发、FIFO 排队、排队超时和 queued 取消。
...
16. 同一 conversation 下两个父 Task 顺序执行：第一项完全结束后第二项才能进入 Core；两者的 shadow、投影、续轮和幂等关联不串线，第一项的陈旧输入或取消不能命中第二项。

验收日志至少包含：`parentTaskId`、`conversationId`、`batchId`、`toolCallId`、`remoteAgentId`、member state、latency；禁止记录敏感 tool arguments 和完整结果正文。当前版本未定义自动重试，不要求记录 `attempt`。
