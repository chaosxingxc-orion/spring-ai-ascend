...
|---|---|---|---|---|---|
| C1 | 统一执行契约 | MUST | 所有适配器经 `AgentHandler` 接入；入口层不出现任何具体框架类型 | `FEAT-002:33` | §2.3.1 |
| C2 | 统一结果流 | MUST | 输出经 `QueryChunk` 流承载，覆盖增量、中断、错误三类；终态由流的结束方式表达 | `FEAT-002:34` | §2.3.3、§4.2 |
| C3 | 框架中立执行上下文 | MUST | 以 `ServeRequest` 为执行输入，消费其七个字段 | `FEAT-002:35` | §2.3.2 |
| C4 | 单 Agent 执行模型 | MUST | 一个实例只承诺服务一个 Agent；**不得承诺按标识在同实例内路由多个** | `FEAT-002:36` | §4.4 |
| C5 | 协作式取消 | MUST | 无独立取消入口；取消**至少要阻止 runtime 继续消费本次执行结果** | `FEAT-002:38` | §2.3.4、§4.3 |
| C6 | 框架中立错误表面 | MUST | 原生异常、传输错误、未知结果一律映射为失败终态；原生错误码存在时应保留 | `FEAT-002:39` | §4.2 |
| C7 | 本地框架适配 | MUST | 托管宿主构建的本地 Agent，会话标识由 runtime 传入 | `FEAT-002:41` | **L2-Func-002b《本地框架适配》** |
| C8 | 远端服务代理 | MUST | 把远端 REST/SSE 服务代理为 Agent，完成双向转换 | `FEAT-002:46` | **L2-Func-002b《远端服务代理》** |
| C9 | 远端地址模板 | MUST | 支持会话标识与部署配置变量替换 | `FEAT-002:47` | **L2-Func-002b《远端服务代理》** |
| C10 | 远端请求头与元数据映射 | MUST | 支持配置头、允许列表内透传、结构化覆盖；**不得默认透传全部元数据** | `FEAT-002:48` | **L2-Func-002b《远端服务代理》** |
| C11 | 远端中断检测 | MUST | 远端流关闭但无明确终止事件时**不得误报完成** | `FEAT-002:50` | **L2-Func-002b《远端服务代理》** |
| C12 | 适配器健康与生命周期 | SHOULD | 应实现启动与停止；**持续健康探测由后续版本补齐** | `FEAT-002:37` | §2.3.1 |
| C13 | 远端结果提取 | SHOULD | 按配置的节点名与节点类型提取业务结果 | `FEAT-002:49` | **L2-Func-002b《远端服务代理》** |
| C14 | 某 JVM 框架的三条适配 | MUST | 本地实例包装、其暂停恢复三类语义 | `FEAT-002:42`、`FEAT-002:43`、`FEAT-002:44` | **不适用**，理由与举证见 §2.2 |
