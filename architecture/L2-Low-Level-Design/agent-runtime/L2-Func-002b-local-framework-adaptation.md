...
| # | 能力 | 级别 | 事实要求 | 权威 | 状态 |
|---|---|---|---|---|---|
| L1 | 本地 Agent 托管 | MUST | 经统一契约托管三种形态的本地 Agent | `:41` | 已实现（形态待收敛，§1.5） |
| L2 | 会话标识传递 | MUST | 用 runtime 传入的会话标识作框架会话来源 | `:41`、`:62` | 已实现 |
| L3 | 执行上下文消费 | MUST | 消费七个字段 | `:35` | **部分实现**，§13 |
| L4 | 输出归一 | MUST | 原生输出经统一结果块承载 | `:34`、`:41` | 已实现 |
| L5 | 协作式取消 | MUST | 至少阻止 runtime 继续消费本次结果 | `:38` | **有缺口**，§13 |
| L6 | 异常映射 | MUST | 框架异常映射为失败终态，不得绕过 | `:136` | **已实现** |
| L7 | 会话清理 | MUST | 提供会话清理入口 | `:59` | **已实现**（两步，§4.3） |
| L8 | 生命周期 | SHOULD | 实现启动与停止 | `:37` | **未接线**，§13 |
...
**未知类型的处置有正反两个实证**：同仓的宿主事件适配做到了「过滤 + 告警」，而本地框架适配当前是**兜底透传且全文件无日志**——后者属权威 `:102` 明禁的静默跳过。**待整改**（§13）。
