...
| # | 能力 | 级别 | 事实要求 | 权威 | 落点 |
|---|---|---|---|---|---|
| C1 | 远端服务代理 | MUST | 把远端 HTTP 服务代理为 runtime Agent，完成请求与响应的双向转换 | `FEAT-002:46` | §4.1 |
| C2 | 地址模板 | MUST | 支持会话标识占位替换，使 runtime 会话稳定映射到远端会话 | `FEAT-002:47`、`FEAT-002:123` | §4.1.1 |
| C3 | 请求头映射 | MUST | 支持配置头、允许列表内透传；**不得默认透传全部** | `FEAT-002:48`、`FEAT-002:124` | §4.1.1 |
| C4 | 中断检测 | MUST | 流关闭但无终止事件时**不得误报完成** | `FEAT-002:50`、`FEAT-002:101`、`FEAT-002:140` | §4.2 |
| C5 | 结果提取 | SHOULD | 按节点名与节点类型提取业务结果；**不得改变终态语义** | `FEAT-002:49`、`FEAT-002:127` | §4.3 |
| C6 | 结构化元数据覆盖 | MUST | 权威列为映射规则之一 | `FEAT-002:48` | **不实现**，见 §2.2 |
