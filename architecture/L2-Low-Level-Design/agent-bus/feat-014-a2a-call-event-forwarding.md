...
本期 4 条运行态决策（与 FEAT-013 同源，见 [`feat-013 §1.1`](./feat-013-client-invocation-event-forwarding.md)）：三单元（gateway / event-bus / registry-discovery-center）独立可替换——设计意图不变（as-built：event-bus relay 为唯一生产进程、gateway 测试源码、registry 未迁，见命名说明）；两跳均经 RocketMQ pub/sub；broker 选型 RocketMQ；**event-bus→agent-runtime 不走 a2a push**（现有 `A2aForwardingDeliveryPort` T1 HTTP push 在本特性范围内被 broker 取代）。
...
| route handle 消费 | MUST | ⬜ | 设计上复用 registry-discovery-center 解析 route handle，不暴露物理 endpoint。as-built（agent-solution）：registry 未迁，route handle 由 `MapEndpointResolver` + `IngressEnvelope.requestAttributes` 解析、topic 由 `BrokerTopicResolver` 派生（见 feat-013 §3.1/命名说明）。 |
...
com.openjiuwen.bus.forwarding.                  # 复用（agent-solution 仓 common/agent-bus/，4 模块；FEAT-014 与 FEAT-013 共享底座）
  spi.                                        #   ForwardingEnvelope(扩展) / AgentBusEventType(含 A2A_CALL_*/A2A_STREAM_* 族) /
    ...
    └── broker.                               #   broker 转发 SPI 端口 + BrokerControlDescriptor（含 originalCaller）
  ...
  runtime.
    ...
    persistence.jdbc.                         #   JdbcForwardingOutbox/Inbox/SqlCodec（V1+V3 migration）
  （registry.runtime 平面未迁移：AgentDiscoveryService.resolveRouteHandle / RouteHandleCodec 不在本仓；route 由 MapEndpointResolver + IngressEnvelope.requestAttributes 解析）
```
...
| 被调用方 runtime（target） | 生产方（响应） | tap `AgentEmitter` → `BrokerForwardingRelayPort.produce` → `T_a2a_resp_in` | `A2A_CALL_ACCEPTED` / `_REJECTED` / `_FAILED` / `_RESPONSE` / `A2A_STREAM_READY` / `A2A_CALL_TERMINAL` |
...
> **as-built 补充（5193972e）**：与 FEAT-013 同构——hop2 `messageId` 用 `eb-`+hop1 前缀（共享 outbox 表防 `ON CONFLICT DO NOTHING` 吞 hop2）；`originalCaller`（调用方 serviceId）随 payloadRef 控制描述符端到端携带，响应跨 relay 回路由。详见 feat-013 §4.1/§4.4。
...
**消费**：
- `BrokerInvocationConsumer`（net-new）`subscribe(consumerServiceId=runtime-<targetServiceId>, route, DeliveryFilter.forRuntime(tenant, targetServiceId))` 固定 `T_a2a_deliver` + 过滤条件，`poll(nowMillisEpoch)`；broker 端 SQL92 过滤 `tenantId`/`targetServiceId`（`supportsBrokerSidePropertyFilter()=true`，不投递跨 tenant 消息）——复用 in-repo `RocketMqBrokerForwardingConsumer`。
...
# topic 不经配置项声明——BrokerTopicResolver 按 ascend_bus_<routeHandle.value()>_<suffix> 派生：
#   a2a 路由 × req/deliver/resp_in/resp_out → ascend_bus_a2a_{req,deliver,resp_in,resp_out}（与 invocation 路由同构，仅 routeHandle.value() 不同）
  ...
  service:
    broker:
      rocketmq:
        ...
        topics:                                   # ⬜ agent-runtime 侧 net-new（设计目标，未实现）；topic 实际由 BrokerTopicResolver 派生，不经配置项
...
> event-bus 侧配置复用 [`feat-013 §5.3`](./feat-013-client-invocation-event-forwarding.md)——flat `agent-bus.*`（`AgentBusBrokerProperties`，无 `broker.rocketmq.` 嵌套、无 `topics` 项）；FEAT-014 与 FEAT-013 共享同一 `application.yml`，仅 `routeHandle.value()`=`a2a` 时 `BrokerTopicResolver` 派生 `ascend_bus_a2a_*` topic（不经配置项声明）。上方 `openjiuwen.service.broker.*` 为 agent-runtime 侧 net-new 设计目标（外仓 in-flight）。
...
//   target=RouteResolution from A2ARemoteAgentCardRegistry → ForwardingEndpointResolver
//   （as-built agent-solution：impl=MapEndpointResolver 配置映射；registry 集成后由 registry-resolver 提供）
...
| route handle 不可解析 | `ForwardingEndpointResolver.resolve(routeHandle)` 返回空 | `BrokerProduceOutcome.ROUTE_NOT_FOUND` → 非 retryable → DLQ | `route_not_found`；**不暴露物理 endpoint 作恢复路径** |
...
- `ForwardingFailureCode`/`ForwardingStatus`/`ForwardingRetryPolicy`/`RouteCircuitBreaker`/`ForwardingEndpointResolver`（impl `MapEndpointResolver`/`BrokerTopicResolver`；`AgentDiscoveryService.resolveRouteHandle`/`RouteHandleCodec` 属 registry 平面，未迁入 agent-solution）。
