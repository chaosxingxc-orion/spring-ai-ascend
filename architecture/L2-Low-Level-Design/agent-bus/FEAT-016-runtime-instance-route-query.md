...
| **多实例候选是正常形态** | scope §2 / §5.1.1；`serviceId` 是逻辑服务标识，可被多实例共享；registry PK `(tenant_id, agent_id, service_id)` 支持同 `agentId` 多实例。 |
...
| 已知目标路由查询（by serviceId / capability） | 按 `serviceId` / `capability` 查询 | （scope §2） | ⬜ 阶段二 |
| route handle 解析 | opaque handle → 物理端点（仅转发层） | `resolveRouteHandle` | ✅ MVP |
| 多实例候选表达 | 同 `agentId` 多实例各自独立 handle | `List<AgentCardDto>` + `(tenant_id, agent_id, service_id)` PK | ✅ MVP |
...
| 中心短时不可用降级 | 本地已知路由信息维持调用 | （本地缓存） | ⬜ 阶段二 |
...
| 已知目标路由查询（serviceId / capability） | ⬜ | MVP 仅支持 agentId 维度；serviceId / capability 维度阶段二引入（capability 字段已在 REQ-2026-004 移除，阶段二重建）。 |
...
| 运行时实例标识 | ✅ | `serviceId`（server-derived `host-port`）区分逻辑服务与具体实例；实例标识进 route handle，不进 DTO 明文。 |
| 路由引用 | ✅ | opaque `routeHandle`（`v1:` + base64 JSON 5 字段）；对 agent / client 不透明。 |
...
| 健康与可用性 | ⚠️ | MVP 提供 `ONLINE` / `DEGRADED` / `DRAINING` / `OFFLINE` 四态；scope §5.1.5 的「可能不可用 / 有限可用」细分状态未实现。 |
| 中心短时不可用降级 | ⬜ | 本地路由缓存 + 有效性窗口——阶段二；MVP 中心不可用时显式失败。 |
...
| 具体缓存结构 / 淘汰算法 / 本地路由策略 | scope §5.2 | 阶段二降级能力一并设计 |
...
> **阶段二扩展**：`searchByServiceId(tenantId, serviceId)` 与 `searchByCapability(...)` 在阶段二随 `capability` 字段重建一并引入；当前不预先钉死方法签名。
...
|------|------|------------------|------|
| `routeHandle` | 否 | 路由引用 | opaque handle，转发层经 `resolveRouteHandle` 解析 |
...
| `frameworkType` | 是 | 业务定义 | `JIUWEN` / `AGENTSCOPE` / `VERSATLE` / `PROXY_SERVICE` |

> **系统路由视图不可见内容**（scope §5.1.4）：Task execution state、agent 内部编排状态。DTO 设计上不含任何 Task 字段。
>
> **路由可用性投影不可见内容**（scope §5.1.4）：候选发现结果、Agent Card 语义画像、物理 endpoint、topic、实例地址、数据库 key、探活实现、`serviceId` 明文、`routeKey` 明文。MVP 的 `AgentCardDto` 已满足：不含 `endpointUrl` / `routeKey` / `serviceId` 明文。投影层（剥离 `weight` / `maxConcurrency` / `frameworkType` 等，只留 `agentId` / 能力 / 简化健康 / 版本可用性）在 `agent-runtime` 对 agent 表面实现，不在 registry 单元。
...
- **必须**：`searchInstancesByAgentId` 返回的 DTO 不携带 `endpointUrl` / `routeKey` / `serviceId` 明文——只有 opaque `routeHandle`。
...
- **中心不可用**：MVP 显式失败；阶段二引入本地缓存后返回 `degraded / cached route`。
...
registry-discovery-center
   ...
   │  PK: (tenant_id, agent_id, service_id)
   │
   │  tenant-A / wealth-expert-01 / 10.0.0.1-8080   ONLINE   weight=100
   │  tenant-A / wealth-expert-01 / 10.0.0.2-8080   ONLINE   weight=100
   │  tenant-A / wealth-expert-01 / 10.0.0.3-8080   DEGRADED weight=80
   │
   │  searchInstancesByAgentId("tenant-A", "wealth-expert-01")
   │   → List<
   │       AgentCardDto{routeHandle=v1:...10.0.0.1..., health=ONLINE, weight=100, ...},
   │       AgentCardDto{routeHandle=v1:...10.0.0.2..., health=ONLINE, weight=100, ...},
   │       AgentCardDto{routeHandle=v1:...10.0.0.3..., health=DEGRADED, weight=80, ...}
   │     >
   │  排序：weight DESC, last_heartbeat DESC
```

- **同 `agentId` 多实例**：正常形态；实现不要求 `agentId` 在实例维度唯一。
- **`serviceId` 区分实例**：server-derived `host-port`，进 route handle，不进 DTO 明文。
...
| 版本不匹配 | DTO 携带 `contractVersion` / `capabilityVersion` | 调用方比对；MVP 不在 discovery 层强制过滤，阶段二可在查询输入加版本约束 |
...
- **降级窗口**：阶段二设计，受控有效性约束（如 handle TTL = 一个探活周期 5s），不承诺长期离线工作。
...
> 阶段二降级能力引入后，将新增 `agent-bus.registry.mvp.local-cache.*` 配置组（TTL / 最大条目 / 淘汰策略），届时在本文 §5 扩展。
...
# 预期：JSON 数组，每项含 opaque routeHandle；不含 endpointUrl / routeKey / serviceId 明文
```
...
# 预期：{"endpointUrl": "http://192.168.1.50:8000", "routeKey": "wealth/v1", "contractVersion": "1"}
...
curl -s -X POST http://localhost:8092/api/registry/route-handle/resolve \
  -H "Content-Type: application/json" \
  -d '{"routeHandle": "v1:eyJ0ZW5hbnRJZCI6InRlbmFudC1BIn0==", "tenantId": "tenant-B"}'
...
| route handle 畸形 | 缺 `v1:` 前缀 / base64 损坏 / JSON 缺字段 / 旧 4 字段格式 | 400 | `malformed_handle` | baseline-breaking |
...
| 中心不可用 + 本地信息可用（阶段二） | 同上 + 本地缓存命中 | 200 | （`degraded` 标记） | 阶段二实现 |
| 版本不匹配 | DTO `contractVersion` 不满足调用方要求 | 200 | （DTO 含版本字段） | 调用方判断并排除；阶段二可在查询输入加版本过滤 |
...
| 无权限目标 | 空 List（反枚举） |
| 目标不存在 | 空 List |
| 中心不可用且本地信息可用 | 阶段二（`degraded / cached route`） |
...
| 限制 | 影响范围 | 临时方案 / 阶段规划 |
|------|---------|-------------------|
| 仅支持 by `agentId` 查询，不支持 by `serviceId` / `capability` | `serviceId` / `capability` 维度的已知目标查询无法直接使用 | 阶段二随 `capability` 字段重建引入 `searchByServiceId` / `searchByCapability` |
| 无中心不可用降级 | 中心短时不可用时显式失败，无法维持已知目标调用 | 阶段二引入本地缓存 + 有效性窗口（handle TTL = 一个探活周期） |
| 可用性状态细分不足 | scope §5.1.5 的「有限可用」未实现；`DRAINING` 在 SQL 层直接排除而非呈现为「有限可用」 | 阶段二评估是否把 `DRAINING` 纳入「有限可用」投影 |
| 版本不匹配不在 discovery 层强制过滤 | 调用方需自行比对 `contractVersion` | DTO 携带版本字段；阶段二可在查询输入加 `contractVersion` 过滤 |
...
| scope 要求 | L2 落地 | 备注 |
|-----------|---------|------|
| §2 已知目标路由查询 MUST | §4.1 / §4.2 / §2.3.1 | MVP by agentId；by serviceId / capability 阶段二 |
...
| §2 多实例候选 MUST | §4.3 | `List<AgentCardDto>` + `(tenant_id, agent_id, service_id)` PK |
| §2 运行时实例标识 MUST | §4.3 / §2.3.2 | `serviceId` 区分实例，进 handle 不进 DTO 明文 |
| §2 路由引用 MUST | §2.3.2 / §4.1 / §4.2 | opaque `routeHandle` |
| §2 路由可用性投影 MUST | §2.3.2 / §4.2 | 投影层在 agent-runtime；registry 输出系统路由视图 |
| §2 租户隔离 MUST | §2.3.3 / §4.6 / §7 | RLS + 应用层 WHERE + TenantContext |
| §2 版本约束 MUST | §2.3.2 / §4.4 | DTO 携带版本字段；阶段二加强制过滤 |
| §2 健康与可用性 MUST | §4.4 | MVP 子集；阶段二补「有限可用」 |
| §2 中心短时不可用降级 MUST | §4.5 / §8 | 阶段二 |
...
| §5.1.6 中心不可用与恢复 | §4.5 | MVP 显式失败；阶段二降级 |
| §5.1.7 错误语义 | §7 | 映射表 |
