...
| 非 Task 查询表面 | Non-Task Query facade | 面向兼容接入和即时调用的同步/SSE 查询表面。它直接驱动一次 Agent invocation，不创建或绑定 runtime Task，不提供 GetTask、CancelTask、SubscribeToTask，也不得伪造可查询的 Task 状态。当前 openJiuwen 实现中的 `/v1/query`、`/query`、`/v1/query/reactive` 及 Feat-Func-022 Custom REST 扩展属于该表面。 | `agent-runtime` ingress / extension assembly | Service Task API、Task query、Task lifecycle owner | active |
