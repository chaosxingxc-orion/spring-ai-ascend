...
模块内文档或实现材料发生冲突时，按以下事实权威优先级处理：

1. 已实现且可验证的源码、架构测试、契约测试、模块元数据和生成证据。
2. 当前 `architecture/` 下对应模块的 SPI/API 附录和显式边界文档。
3. 当前 `architecture/` 下对应模块的 L1/L2 视图文档、README 和术语表。
4. `version-scope/` 中的当前版本范围事实。
5. `docs/architecture/` 下尚未吸收到 active 架构目录的提案、草案和评审材料。
6. `docs/archive/`、历史日志和外部记忆。

当低优先级材料与高优先级事实冲突时，应修改或标注低优先级材料；当代码事实与 active 架构事实冲突时，应显式反馈到对应架构层级或创建治理事项，不得静默选择其一。
