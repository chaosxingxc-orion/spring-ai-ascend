...
| **Principle 3:<br>显式能力放置与数据边界** | 企业级数据隐私与权限是第一红线。工具、上下文、记忆、审批 UI 等能力，必须**显式声明其执行宿主与所跨越的数据边界**。 | 1. 单个 `agent-runtime` 实例内的多智能体协作由该实例内部闭合。<br>2. 跨实例、跨数据边界的 A2A（Agent-to-Agent）控制流与数据交互，**必须强制经由中央 `agent-bus`（智能体总线）**。 |
...
* **策略与约束**：
  * **全链路异步挂起**：基于 `agent-core` 与 `agent-runtime` 之间受治理的执行契约，长等待任务必须持久化至状态机，释放物理线程。
