...
| FEAT-013 | 客户端调用事件转发（gateway↔event-bus） | Gateway + 真 bus | L2 设计态（draft） | gateway↔event-bus 客户端调用与响应事件转发；投影到 AB-F11 转发底座 + gateway 单元；本期 RocketMQ pub/sub 两跳、event-bus 治理中继。见 [`feat-013`](../../../L2-Low-Level-Design/agent-bus/feat-013-client-invocation-event-forwarding.md)。 |
| FEAT-014 | A2A 调用事件转发（event-bus↔agent-runtime） | 真 bus | L2 设计态（draft） | event-bus↔agent-runtime 服务间 A2A 调用事件转发；投影到 AB-F11 转发底座 + agent-runtime consumer/producer；本期取代 a2a push。见 [`feat-014`](../../../L2-Low-Level-Design/agent-bus/feat-014-a2a-call-event-forwarding.md)。 |
