...
- 真实持久化（Stage 12 JDBC adapter + Flyway + RLS）与真实投递绑定（Stage 15 A2A transport adapter）已落地，Stage 17/18 端到端验证通过 + Stage 19 重投往返生命周期端到端验证通过 + Stage 20 验证回填（租约过期 reclaim + 断路器真实链路）端到端通过 + Stage 21 多 worker 并发验证端到端通过 + Stage 22 时间驱动的终态与续约端到端验证通过（EXPIRED 终态 markExpired 落盘 + lease 续约真实触发）+ Stage 23 payloadRef 端到端传递验证通过（DATA_BEARING payloadRef 真实 PG round-trip 不变 + A2A metadata 传输边界两分支对称）+ Stage 24 RLS 接线（adapter 首次引入 `TransactionTemplate` 事务管理，`withTenant` 在每端口方法事务内 `set_config('app.tenant_id',…,true)` ≡ SET LOCAL 激活 §7.3 fail-closed RLS，闭合 Stage 12「armed but not wired」；有生产代码改动但生产依赖边界不变）（200 tests green）；**仍 deferred**：真实 queue / replay store 物理实现、push vs pull / 是否引 MQ 的最终投递模型裁决（独立 H2/H3 议题）、registry 集成的 resolver 生产实现、连接池治理。
