...
- 类 MQ 转发底座 runtime 的真实投递 / broker / queue / DLQ / replay 存储（Stage 12 已落地 JDBC adapter + Flyway migration + RLS 持久化层；Stage 15 落地真实 A2A 投递绑定 PoC；Stage 25 裁决投递模型 T4 hybrid[outbox + broker]，解除 §6.1 第 1 项引 broker 圈 `transport.broker` 子包，broker 产品选型 / 物理接线 deferred Stage 26+ PoC）。
