...
| `bus.forwarding.spi.broker` | broker 转发 SPI 端口（`BrokerForwardingRelayPort`/`ConsumerPort` + `BrokerInboundMessage`/`BrokerProduceOutcome`/`DeliveryFilter`）+ `BrokerControlDescriptor`（payloadRef routing-descriptor codec，含 `originalCaller`；gateway-assembly-purify 从 `runtime.transport.broker` 迁入，pure Java） | 已落地（Stage 26 SPI + ADR-0164 迁入） |
