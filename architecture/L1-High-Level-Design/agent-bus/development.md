    ...
    forwarding/
      ...
      runtime/    # C3 转发状态机 + dispatcher worker + dispatch loop + retry policy + circuit breaker + EpochClock（Stage 7 + Stage 8 + Stage 10 + Stage 11 + Stage 14 + Stage 16，纯 Java）
        ...
        transport/         # Stage 15: 真实投递绑定（A2aForwardingDeliveryPort + ForwardingEndpointResolver）；transport 抽象 + a2a 具体实现
          ...
          broker/          # Stage 26: broker 消息类型 + rocketmq adapter（SPI 端口在 spi.broker，InMemoryBroker 在 testkit）；锁定 RocketMQ，FEAT-013/014 落地 adapter
    spi/
      engine/
      federation/
      ingress/
...
| `bus.forwarding.runtime.transport.broker` | broker 消息类型（`BrokerClientProperties`/`BrokerOutboundMessage`/`BrokerMessageHeaders`，与 `spi.broker` 的 inbound 类型对称）+ `rocketmq` 子包 adapter（`RocketMqBrokerForwardingConsumer`/`Relay` + `RocketMqBrokerClientConfiguration` role-agnostic 客户端 bean：`defaultProducer`/`requestRelay`("req")/`responseConsumer`("resp_out")）；`InMemoryBroker` 契约测试在 `agent-bus-testkit` | 已落地（Stage 26 SPI + FEAT-013/014 RocketMQ adapter + E2E IT，443 green / 14 skip） |
| `bus.forwarding.spi.broker` | broker 转发 SPI 端口（`BrokerForwardingRelayPort`/`ConsumerPort` + `BrokerInboundMessage`/`BrokerProduceOutcome`/`DeliveryFilter`；P-06：`BrokerInboundMessage` 控制面 traceId/idempotencyKey/routeHandle/capability/deadline/inlinePayload/originalCaller 为一级字段，`payloadRef` 纯 A2A 数据引用，**无 descriptor codec**——`BrokerControlDescriptor` 已删除） | 已落地（Stage 26 SPI + ADR-0164 迁入 + P-06 控制面/数据面分离） |
| `bus.forwarding.common` | 共享 infra bean 装配（de-gateway-ification，ADR-0164 follow-on）：`AgentBusInfrastructureConfiguration`（无 `@Profile`，两进程共用 `brokerClientProperties`/`forwardingOutbox`/`forwardingInbox`）+ `AgentBusBrokerProperties` | 已落地（FEAT-013/014） |
...
- concrete broker / MQ **圈禁在 `bus.forwarding.runtime.transport.broker..` 子包**（FEAT-013/014 落地 RocketMQ client，ArchUnit rocketmq-import 规则覆盖 `transport.broker..`；`AgentBusForwardingSpiPurityTest` 强制）。**Stage 12 精确化**：Spring JDBC / Flyway / Postgres driver 圈在 `bus.forwarding.runtime.persistence.jdbc` 子包；`bus.forwarding..` 主体仍纯 Java（`forwarding.common` / `forwarding.spi` / `forwarding.spi.broker` 纯 Java，仅 `@ConfigurationProperties` / `@Configuration` Spring 装配豁免）。
