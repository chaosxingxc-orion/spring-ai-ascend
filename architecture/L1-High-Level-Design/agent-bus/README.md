...
> **FEAT-013/014 L2 已为本期决定接线方向**（as-built）：三单元（gateway / event-bus / registry-discovery-center）独立可替换——设计意图不变；gateway→event-bus、event-bus→agent-runtime 两跳均经 RocketMQ pub/sub；event-bus→agent-runtime 不走 a2a push（现有 `A2aForwardingDeliveryPort` T1 push 在本特性范围内被 broker 取代）。as-built（agent-solution 仓 `common/agent-bus/`，4 模块、`com.openjiuwen.bus`、`EventBusRelayApplication`）：event-bus relay 为唯一生产进程，gateway 运行时为测试源码，registry 平面未迁移（route 由 `MapEndpointResolver` 解析）。该方向推进 Stage 26 "broker 物理接线 / relay adapter / receiver consumer / `AWAITING_ACK` 状态机 deferred Stage 27+" 在 FEAT-013/014 范围内的落地（非推翻 Stage 叙事）。见 [`feat-013`](../../L2-Low-Level-Design/agent-bus/feat-013-client-invocation-event-forwarding.md) / [`feat-014`](../../L2-Low-Level-Design/agent-bus/feat-014-a2a-call-event-forwarding.md)。
