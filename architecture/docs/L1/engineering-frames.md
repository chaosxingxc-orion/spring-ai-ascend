...
|---|---|
| `agent-service` (compute_control) | `EF-ACCESS-ADMISSION`, `EF-SESSION-TASK-STATE`, `EF-TASK-CONTROL`, `EF-ENGINE-DISPATCH`, `EF-INTERNAL-EVENT-QUEUE` (design-only), `EF-TRANSLATION-INTERCEPT` |
| `agent-bus` (bus_state) | `EF-INGRESS-GATEWAY`, `EF-S2C-TRANSPORT`, `EF-CHANNEL-ISOLATION` |
| `agent-execution-engine` (compute_control) | `EF-ENGINE-REGISTRY`, `EF-ORCHESTRATION-SPI` |
