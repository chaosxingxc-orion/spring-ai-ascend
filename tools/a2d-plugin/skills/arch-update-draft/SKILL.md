...
5. **渲染审阅 HTML**:
   ```bash
   python skills/_shared/renderer/render.py <draft.yaml> <output.html> --mode full
   ```
   > 注:MVP 阶段 step 3 与 step 8 共用 full 视图渲染。delta 视图(左基线右本轮、标红/绿变更)为后续切片,当前用 `status: draft` 标记区分本轮待落地的元素。
