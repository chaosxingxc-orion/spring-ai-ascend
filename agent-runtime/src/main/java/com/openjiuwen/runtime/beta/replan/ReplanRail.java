 ...
 *
 * <p>形态：1.0 ReActAgent 无 2.0 的 LLMDecision.Replan 语义，用 {@link ReplanTool}
 ...
 * </ul>
 *
 * <p>为何用 afterModelCall 而非 afterToolCall：{@code afterToolCall} 的 forceFinish <b>不拦</b> tool_call
 * 循环（offset 878 在 interrupt 路径，正常循环不消费），故用 {@code afterModelCall}（offset 700，callModel
 * 后、toolCalls 终态判定前消费，能拦）——与 {@link com.openjiuwen.runtime.beta.verification.CriteriaVerificationRail}
 * 同一 afterModelCall forceFinish 路径。
 *
 * <p>核心契约（IFF）：Replan 计数 ⟺ 超限 escalate。剥 withReplan 计数 → canReplan 永真 →
 * 永不 forceFinish → 测试 RED 证非恒真。
 *
 * <p>诚实边界：超限 escalate 是 degraded 终态（诚实标记，不假装能修正——afterModelCall gate 结构上
 * 无法强制修正，同 criteria 验证 rail）。replan 后"重新生成计划"（beta 的 planGenerator.generate）在 1.0
 * 无 BetaPlan 对应物，defer。多 session 并发同一 rail 实例的计数串扰用 synchronized 防御
 * （per-agent rail 一般无此问题）。
 *
 * <p>resource_mgr execute 接线：{@link ReplanTool} 的 execute（invoke 被调）经 {@code Runner.resourceMgr()}，
 * 完整嫁接需 {@code Runner.resourceMgr().addTool} 注册（同 OpenJiuwenRemoteToolInstaller 范式）。
 * 本 rail 的计数/超限 forceFinish 在工具 execute 前发生（offset 700 早于 executeToolCall offset 871），
 * 不依赖 execute invoke——但未超限放行后的工具 execute（LLM 看到 ReplanTool 返回的确认）需 resource_mgr
 * 注册才工作。
 *
 * <p>实现偏差（vs 原 spec）：原 "移植 DecisionParser + LLMDecision sealed 8 态"缩范围——1.0 ReActAgent
 * 用原生 tool_call/answer 语义（无 JSON 决策解析落点），DecisionParser defer；LLMDecision sealed 8 态
 * 已全移（criteria/Replan 共用），Replan 经虚拟工具承载（非 DecisionParser JSON 解析）。
 */
public class ReplanRail extends AgentRail {
    ...
    private static GoalSpec.ReplanRecord extractReplanRecord(ToolCall replanCall) {
        ...
        // stepIndex 简化为 0——精确步骤追踪非本 rail 职责，defer
