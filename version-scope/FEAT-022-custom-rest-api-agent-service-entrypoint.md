...
本特性不是外部网关或 BFF 的实现方案，也不是替代 A2A JSON-RPC 的系统间标准协议。它是 `FEAT-001` 标准 Agent 服务入口的边缘服务化扩展：自定义 REST 入口可以改变 URL、请求字段、响应信封和 SSE 展示形态，但不得改变 Agent 执行、Task 生命周期、状态、错误和租户上下文。
...
| A2A 语义归一 | MUST | 现有约束 | 自定义 REST 入口的执行、状态、错误、超时、取消边界和租户上下文必须归一到 `FEAT-001` 标准 Agent 服务入口语义。 |
...
| correlation / trace 可观测链路 | OUT | 不在范围 | 当前版本不新增或验收 correlation、trace、日志与审计链路；相关字段仍可由用户作为普通 metadata 映射。 |
...
| 入口 / 对象 | 类型 | 事实要求 |
|---|---|---|
| 自定义 REST path pattern | HTTP endpoint | 必须由用户声明并由 runtime 暴露；当前版本只支持一个 path pattern。 |
| HTTP method | HTTP contract | 当前版本主路径应支持消息提交类请求；具体 method 集合由 L2 定义，但不得把 Task 查询、取消或异步能力写成当前版本 MUST 之外的事实。 |
| Request header | HTTP metadata | 可作为租户、用户、空间、鉴权透传或业务 metadata 的输入来源；本特性不声明完成认证授权。 |
...
| JSON response body | HTTP body | 同步响应必须按用户定义信封返回，同时保持结果、错误和Task引用等语义可追溯到标准Agent服务入口。 |
...
|---|---|---|---|
| 启用自定义 REST 入口 | runtime 已 host 一个 Agent，用户已提供自定义映射与响应投影扩展 | 部署方启用该入口并配置一个 path pattern | runtime 暴露一个自定义 REST 入口；路径冲突或入口配置不完整时，该入口不应以半可用状态对外提供服务。 |
...
| 传递业务上下文 | 调用方在 path、query、header 或 body 中携带业务字段 | 用户自定义映射把相关字段转为标准上下文或 metadata | 映射后的字段进入标准 Agent 调用上下文或 metadata；字段不会改变单 runtime 单 Agent 边界。 |
| 外部字段冲突 | 同一业务语义在多个请求来源中出现不同值 | 用户自定义映射规则自行选择、合并、拒绝或透传 | runtime 不提供统一优先级承诺；调用结果以用户扩展定义为准，但不得破坏标准 Task和错误语义。 |
...
- 自定义 REST 入口可以改变请求和响应外观，但不得改变底层 Agent 执行、Task 生命周期、状态推进、错误分类和租户上下文。
...
| tenant 上下文 | 自定义 REST 调用必须把用户映射产生的 tenant传入标准 Agent执行请求，并用于 conversation/Task隔离。 |
...
| 多个自定义 REST path | 当前版本不承诺同一 runtime 内存在多个独立或兼容的自定义 REST path。 |
...
| correlation / trace 可观测性 | 当前版本不承诺新增 correlation、trace、日志或审计关联能力。 |
...
- 自定义 REST 入口必须复用标准 Agent 执行、Task、错误、取消、中断和租户上下文，不得建立私有执行链路或私有状态 owner。
- 当前版本只允许一个自定义 path；该 path 不得引入多个 hosted Agent 或同实例多 Agent 路由。
- 用户自定义映射规则可以自行处理字段冲突、字段透传和响应信封，但实现文档必须说明其选择，避免测试和验收依赖隐式行为。
- 若未来要支持多个独立或兼容的自定义 REST path、完整 Task 管理 REST surface、异步提交规范、webhook、multipart、二进制文件、批量消息、独立 run resource、多 Agent 路由或认证授权协议，必须先更新本特性或新增 version-scope 特性文档，再进入 L2 和实现。
...
- `architecture/L2-Low-Level-Design/agent-runtime/Feat-Func-022-custom-rest-api-agent-service-entrypoint.md`
