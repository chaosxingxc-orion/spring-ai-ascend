...
本特性不替代 `FEAT-001` 定义的标准 Agent 服务入口，也不替代 `FEAT-004` 中 runtime 侧远程 Agent 编排的本地回灌、中断续接和工具化适配。`agent-runtime` 仍拥有本地 Task 生命周期和远端 Task 引用的本地绑定关系；被调用方 `agent-runtime` 仍拥有远端 Task 生命周期、Task 状态和 A2A SSE 流；`agent-bus` 只治理跨服务事件转发和窄控制通道。
