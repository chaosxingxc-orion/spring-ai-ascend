...
本特性不是外部网关或 BFF 的实现方案，也不是替代 A2A JSON-RPC 的系统间标准协议。它与 `FEAT-001` 标准 Agent 服务入口关联，复用相同的 runtime 内部执行基础；自定义 REST 入口可以改变 URL、请求字段和响应信封，但属于非 Task Query facade，不创建正式 A2A Task，也不扩展 A2A Task 生命周期。
...
| 内置入口共存 | MUST | 新增 | 自定义 REST 入口不替换或禁用 `/a2a` 与 `/v1/query`；内置固定入口不计入“一个自定义入口”约束。 |
...
| 同步消息调用 | MUST | 新增 | 自定义 REST 入口必须支持一次普通 HTTP 请求对应一次 Agent 消息提交，并以 JSON 响应返回 `QueryResponse.result` 原始结果或框架脱敏错误信息的自定义投影。 |
| 流式消息调用 | MUST | 新增 | 自定义 REST 入口必须支持以 SSE 返回执行过程；runtime 的每个原始流式 chunk 都交给用户 adapter 转换为自定义 SSE data。 |
| Runtime 执行语义归一 | MUST | 现有约束 | 自定义 REST 入口必须复用 runtime 标准请求、同步响应、流式 chunk 和内部编排语义，但不归一为正式 A2A Task 生命周期。 |
...
| JSON response body | HTTP body | 同步响应必须按用户定义信封返回；成功分支向 adapter 提供 `QueryResponse` 原始结果，错误分支提供框架脱敏后的错误信息。 |
| SSE stream | HTTP response | 流式响应必须使用可解析的 SSE；adapter 接收 runtime 原始 chunk 并返回自定义 data body，框架负责 JSON 序列化和 SSE framing。 |
...
上述字段均由用户 adapter 自定义；示例中的 `task_id` 只是自定义信封字段，不表示框架创建或返回正式 A2A Task。
...
| 同步调用 Agent | 自定义 REST 入口已启用，调用方发送非流式请求 | 调用方向配置的 REST path 提交 JSON body | runtime 将请求映射为标准 Agent 调用，返回自定义 JSON 信封。 |
| 流式调用 Agent | 自定义 REST 入口已启用，调用方请求流式响应 | 调用方向配置的 REST path 提交请求并声明流式偏好 | runtime 返回 SSE stream；原始 chunk 由 adapter 包装为自定义 data body，框架逐帧输出。 |
...
| 原入口共存 | 自定义 REST 入口已启用 | 调用方访问 `/a2a` 或 `/v1/query` | 原有固定入口保持可用，不受单个自定义 path 约束。 |
...
- 每次自定义 REST 消息提交都必须归一为一次 runtime 标准 Agent 执行请求。
- 自定义 REST 入口可以改变请求和响应外观，但不得改变底层 Agent 执行、错误分类、租户上下文和可观测事实。
- 自定义 REST 入口不得绕过 runtime 标准执行链路形成私有执行路径，也不创建正式 A2A Task。

#### 5.1.2 单入口单路径语义

- 当前版本只允许一个自定义 REST 入口能力和一个 path pattern。
- `/a2a` 与 `/v1/query` 是内置固定入口，不计入该数量约束。
...
- 非流式请求应返回一次 JSON 响应。
- runtime 原始同步结果交给 adapter，由用户自定义响应信封字段名和投影方式。
- 同步响应不表示框架创建了正式 A2A Task。

#### 5.1.5 流式响应语义

- 流式请求应返回 SSE。
- 每个 runtime 原始 chunk 都交给 adapter，SSE data 字段结构和信封字段由用户定义。
- 收到 interrupt 时只包装当前原始返回；是否结束流由 runtime observer 的 terminal callback 收束。
...
| Agent 执行失败 | 返回自定义错误信封；不得把失败包装为成功结果。 |
| 流式过程中失败 | 应通过 adapter 包装的错误 data body 表达失败，并结束当前流；同一调用最多输出一帧错误。 |
| 中断 | runtime 原始中断返回交给 adapter 包装，不升级为正式 `INPUT_REQUIRED` Task。 |
...
| 正式 Task surface | 不提供正式 Task 创建投影、查询、取消、轮询、订阅或断线恢复。 |
...
- L2 设计必须把 FEAT-022 设计成与 `FEAT-001` 关联、复用 runtime 内部标准执行入口的非 Task REST facade。
- L2 可以定义 Java SPI、配置属性、path 注册、请求解析、响应封装、路由装配和默认实现，但不得把这些实现细节反向提升为本文没有声明的版本事实。
- 自定义 REST 入口必须复用 runtime 标准执行请求、同步响应、流式 chunk、错误、中断和既有可观测链路，不得建立私有执行链路或私有状态 owner。
- 当前版本只允许一个自定义 path 和一个 adapter；内置 `/a2a`、`/v1/query` 入口不受影响。
...
- `architecture/L2-Low-Level-Design/agent-runtime/Feat-Func-022-【自定义智能体服务入口】自定义 RestAPI 到 runtime内部编排层协议适配转换 SPI.md`
- `openJiuwen/agent-solution/common/example/agentcore-ext-remote-a2a-tool-demo/README.md`
