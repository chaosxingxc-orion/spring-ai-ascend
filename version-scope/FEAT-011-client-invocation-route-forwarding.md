...
FEAT-2026-011 定义 `agent-gateway` 当前版本作为客户端统一 A2A 调用路由转发入口的事实：Gateway 必须接收 `agent-client` 封装后的 A2A 兼容 C/S 请求，完成认证鉴权、租户识别、请求标准化、路由决策和 HTTP/SSE 桥接，再把请求投递到目标 `agent-runtime` 标准入口。
