...
本特性解决的问题是：父智能体需要把部分工作委托给一个或多个专业下游智能体执行，但客户端不应直接理解下游 Agent 地址、A2A 调用细节、远端 Task 生命周期或结果汇聚过程。`agent-runtime` 必须以父 Task 为中心完成远程调用代理、状态关联、结果聚合和父任务恢复，使单调用和并行调用在客户端表面保持一致、可追踪、可诊断。
...
| 远端 Task ID 关联 | MUST | 当下游 runtime 受理调用并返回远端 Task 标识时，本地 runtime 必须在内部把该标识与父 Task、对应 `toolCallId` 关联，用于后续远端续轮路由和 runtime 诊断。该标识不要求进入父 Task 投影，也不面向客户端暴露。 |
| 结果与错误并存 | MUST | 同批次中部分下游调用失败、拒绝或超时时，runtime 必须保留成功项和失败项，并把它们作为对应 `toolCallId` 的结构化结果回填。 |
...
| 可观测性 | SHOULD | runtime 应记录父 Task、下游目标、`toolCallId`、状态变化、耗时、结果类别和错误原因；远端 Task 标识只保留在 runtime 内部关联状态中，并遵守 DFX-001 的脱敏要求。 |
...
|---|---|---|
| 标准 A2A Task 入口 | FEAT-001 | 客户端仍通过标准 Task 创建、查询和订阅入口观察父 Task；FEAT-004 不新增客户端调用入口。 |
...
| 本地任务关联表面 | agent-runtime | 父 Task 可观察到下游调用正在执行、已完成、失败或超时等关联状态；具体存储形态由技术详设决定。 |
...
- 父 Task 在等待远程调用期间应保持可查询、可订阅。
- 客户端应能观察到父 Task 正在等待下游智能体调用，以及每个下游调用的目标、关联标识、当前阶段和结果类别。
- FEAT-004 不新增 A2A Task 状态。父 Task 只使用 FEAT-001 / L1 架构定义的标准状态，如 `WORKING`、`COMPLETED`、`FAILED`、`INPUT_REQUIRED`、`REJECTED`。
...
- 下游调用的状态变化、进度、耗时和错误类别可投影到父 Task 的可观测表面；远端 Task 标识属于 runtime 内部路由状态，不进入客户端可见投影。
...
- 测试应覆盖单个下游调用、多个下游调用并行、完成顺序乱序、部分失败、远端拒绝、超时、远端 Task ID 关联、结果按 `toolCallId` 回填和父任务单次恢复。
...
- `version-scope/FEAT-019-parallel-downstream-agent-tasks-generation-and-handoff.md`
