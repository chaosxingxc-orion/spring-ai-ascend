...
FEAT-003 定义 `agent-runtime` 使用 Redis 缓存智能体任务状态时的统一接入要求。该特性新增标准化 Redis 缓存 SPI，使运行时与开发框架能够复用同一 Redis 连接池和操作接口，当前版本支持缓存 A2A Task、Agent 执行 checkpoints，并允许 DeepAgent 执行期通过注入的 BaseKVStore 复用同一 Redis 接入能力自治持久化 Todolist 状态。不同 Redis 使用方能够在原生 Redis 单机版、原生 Redis 集群版以及客户自封装 Redis 组件之间通过统一配置和 SPI 适配切换。

本特性解决的问题是：Agent 在工行现场当前使用原生 Redis 单机版，但工行有自封装 Redis 组件。为满足客户技术规范，Redis 资源的使用需要被客户既有平台统一管理和监控；同时产品不能把业务代码、A2A Task 存储、Agent 状态持久化或未来中间件能力直接绑定到某一个 Redis 客户端实现。此外，DeepAgent 的 Todolist 能力在执行期需要通过注入的 Redis 连接自治持久化规划状态，使 runtime-hosted DeepAgent 在分布式部署中获得持久化能力，不依赖本地文件系统。
...
- DeepAgent 适配开发者：在执行期将 BaseKVStore 实例注入 DeepAgent，使 Core 的 Todolist 能够复用 Redis 连接自治持久化。
- 运维人员：通过启动日志确认当前生效的数据源策略和关键连接配置。
- 集成测试团队：按验收标准验证原生 Redis 单机版、集群版、配置切换和 DeepAgent Todolist 执行期 Redis 持久化行为。

本特性只定义 Redis 数据源的选择、统一操作接口、状态缓存 TTL 基本语义、配置切换、日志脱敏、DeepAgent Todolist 执行期 Redis 持久化的协作边界，以及验收边界。Redis 本身的部署、容灾、数据持久化、监控平台建设、客户封装 JAR 的内部实现、客户私有安全策略和 agent-core 独立样例程序的本地存储策略不属于 `agent-runtime` 当前版本承诺。
...
| 状态缓存 TTL | SHOULD | 现有 | Redis-backed A2A Task 快照和 Agent checkpoints 应支持统一的可配置 TTL，并提供简单默认值，避免状态缓存长期无界增长。DeepAgent Todolist 的 TTL 由 Core 侧或 Redis 侧统一管理，不作为本特性的独立 TTL 配置项。 |
...
| DeepAgent 执行期 Todolist 自治持久化 | MUST | 新增 | 在 Task 连续执行过程中，Core/DeepAgent 通过注入的 BaseKVStore（Redis 连接）自治完成 Todolist 持久化：每次 Todolist 变更操作自动将当前状态写入存储后端，不依赖 runtime 在每次变更时介入。runtime 提供 Redis 连接复用，不解释或接管 TodoItem 领域模型。 |
...
| Redis 状态缓存 TTL | configuration | 应支持为 Redis-backed A2A Task 快照和 Agent checkpoints 配置秒级 TTL；当前版本使用简单统一 TTL，不要求复杂 eviction 策略。DeepAgent Todolist 的 TTL 由 Core 侧或 Redis 侧统一管理。 |
...
| 使用方能力 | runtime consumer | 当前版本的 Redis 使用方范围是 A2A Task 存储、Agent 状态持久化、DeepAgent 执行期 Todolist Redis 存储；所有 Redis 使用方只依赖统一 Redis 操作接口或其注入的 BaseKVStore，不直接依赖某个 Redis 客户端。 |
...
### 4.2 DeepAgent Todolist 执行期自治持久化（新增）

runtime-hosted DeepAgent 提供 Todolist 规划能力时，Core 通过注入的 BaseKVStore（Redis 连接）自治管理 Todolist 状态的持久化。

| 场景 | 前置条件 | Runtime 行为 | Core / DeepAgent 行为 |
|---|---|---|---|
| DeepAgent 执行期 Todolist 持久化 | runtime 已将 Redis 连接注入 DeepAgent | runtime 将 BaseKVStore 实例注入 DeepAgent（`setKvStore()`），不介入每次 Todolist 变更 | Core/DeepAgent 的 TodoTool 在每次 create/modify 操作时自动将 Todolist 状态写入注入的存储后端，不依赖本地文件系统 |
| Todolist 存储后端选择 | DeepAgent 配置了 todoStorageType 和 KV 连接 | runtime 提供 Redis 连接，Core 通过 TodoStorage SPI 选择存储后端 | Core 根据配置决定使用 file 或 kv 后端；kv 后端复用注入的 BaseKVStore 完成读写 |
...
- DeepAgent Todolist 存储以 sessionId 为隔离粒度（sessionId 对应 Task 维度），继承租户前缀。不提供子任务级物理隔离。具体 key schema 属于 Core 内部实现细节，不作为外部稳定契约。
...
- Todolist 持久化验收必须验证：DeepAgent 执行期通过注入的 BaseKVStore 复用 Redis 自治 save/load，每次 Todolist 变更自动持久化到存储后端。
...
| Todolist 领域模型治理 | 不承诺 runtime 理解、校验、合并或迁移 TodoItem JSON；Todolist payload 属于 Core/DeepAgent 内部契约，runtime 不解释其内容。 |
| Todolist 跨 Task 共享 | 不承诺 Todolist Redis 数据跨 Task 共享；Todolist 隔离粒度为 sessionId 对应 Task 维度。 |
| 异常全过程恢复 | 当前版本不承诺 DeepAgent 每一步执行过程的完整异常恢复或精确重放。 |
...
- L2 设计必须分别明确 A2A TaskStore 内部 key、开发者定义的 Agent checkpoint 或业务 key、DeepAgent Todolist 存储 key，以及不同逻辑 runtime 共用 Redis 时的 keyspace 隔离责任；不得把多租户隔离与跨逻辑 runtime 隔离合并描述。
...
- L2 设计必须明确 DeepAgent Todolist 的 runtime/Core 协作边界：runtime 通过 `DeepAgent.setKvStore()` 注入 BaseKVStore 实例；Core 在执行期通过注入的存储后端自治完成 Todolist save/load，每次变更自动持久化；runtime 不接管 TodoItem 领域模型。
- 测试必须覆盖 Todolist 持久化：Core 通过注入的 BaseKVStore 复用 Redis 自治 save/load，每次 Todolist 变更自动持久化到 KV 后端。
- DeepAgent Todolist 存储以 sessionId 为隔离粒度，由 Core 的 KvTodoStorage 管理 key 格式；不提供子任务级物理隔离。
