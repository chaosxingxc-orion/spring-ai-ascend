...
| FEAT-004 | agent-runtime | active | 远程 Agent 编排 | runtime 作为 A2A 客户端接入远程 Agent，基于 Agent Card 生成本地工具，并支持单个或同轮多个下游 Agent 委托的远程调用、结果回填、取消传播和父 Task 恢复。 | [FEAT-004-remote-agent-orchestration.md](./FEAT-004-remote-agent-orchestration.md) | `architecture/L2-Low-Level-Design/agent-runtime/Feat-Func-004-remote-agent-orchestration.md` |
...
| FEAT-022 | agent-runtime | active | 自定义 REST API 服务入口 | runtime 在标准 Agent 服务语义之上提供自定义 REST edge adapter，使调用方以业务 REST/SSE 形态访问同一个 hosted Agent，同时保持 Task、错误、租户和可观测语义归一到 FEAT-001。 | [FEAT-022-custom-rest-api-to-a2a-jsonrpc-adaptation-spi.md](./FEAT-022-custom-rest-api-to-a2a-jsonrpc-adaptation-spi.md) | 待补充 |
