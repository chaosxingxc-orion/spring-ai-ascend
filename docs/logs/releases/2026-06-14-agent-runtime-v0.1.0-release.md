...
**5.3 ✅ 轨迹能力补齐（v0.2.0 已交付）**

- ✅ 轨迹数据对调用方可见（`trajectory.northbound=true` 北向 A2A artifact）
- ✅ OpenTelemetry 导出（OTLP），gen_ai.* 语义约定对齐
- ✅ 首 Token 延迟（TTFT）观测：`MODEL_CALL_FIRST_TOKEN` + `gen_ai.server.time_to_first_token`
- ✅ LLM 推理过程（REASONING）独立事件记录
- ✅ 采样率控制（头部 Bernoulli 采样，整条轨迹 in/out）
- ✅ 大载荷外置存储（`PayloadRefStore` SPI + `LocalFsPayloadRefStore`）
- ✅ 自定义脱敏逻辑注入（`Redactor` SPI + `PatternRedactor`；`CostCalculator` + `TableCostCalculator`）
- ✅ NDJSON 结构化日志轨道（`NdjsonTrajectorySink`，生产 Runbook 见 guides/）
