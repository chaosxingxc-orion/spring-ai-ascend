...
|---|---|---|---|---|---|---|
| `tenantId` | string | 否 | request metadata `tenantId` | request metadata > legacy `message.metadata.tenantId` > `default` | `RuntimeIdentity`、日志、Memory scope、State key | 普通透传值，不做认证、隔离或 RLS |
...
兼容期内，runtime 可以从 `message.metadata` 读取 `tenantId`、`userId`、`agentId`、`agentStateKey`、`stateKey` 作为 fallback，但必须打 deprecation log，方便样例和外部调用方迁移。
...
2. legacy `message.metadata.tenantId`
3. `default`

说明：

- 第 1 层是唯一推荐入口，对应 wire path `params.metadata.tenantId`。
- 第 2 层只用于兼容旧样例或旧调用方，读取时应输出 deprecation log。
...
| request metadata 缺失，message metadata 提供 `tenantId` | legacy fallback 生效，并输出 deprecation log |
...
这样可以减少 OpenJiuwen、AgentScope、Memory、MCP 等能力之间的 metadata 冲突，也为后续业务自定义权限、工具审计和长期记忆治理留下清晰边界。Versatile adapter 的 metadata 迁移暂不纳入本轮，后续可在同一分层原则下单独设计。
