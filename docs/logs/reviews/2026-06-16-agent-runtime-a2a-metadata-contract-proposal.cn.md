...
- `tenantId` 主要通过 `X-Tenant-Id` 传播，L2 文档已经写明 runtime 只传播、不认证。
- 部分样例把 `userId`、`agentId`、`sessionId` 放进 `message.metadata`。
- Memory 样例中存在 `stateKey`、`agentStateKey`、长期记忆 scope 混用的倾向。
...
1. 本轮不迁移 Versatile adapter metadata。
2. 本轮不设计多租户鉴权模型；`tenantId` 仍由上游网关或部署平台保证可信。
3. 本轮不改变 A2A 标准 message schema。
4. 本轮不要求所有历史调用方立即迁移；代码落地时应保留短期 legacy fallback。
...
|---|---|---|---|
| Transport Context | 网关、租户、认证、传输上下文 | `X-Tenant-Id` | 否 |
| A2A Message Body | 业务消息本体和 A2A 标准上下文 | `role`、`parts`、`contextId`、`messageId` | 是 |
| Runtime Metadata | runtime identity、state key、memory scope | `userId`、`agentId`、`agentStateKey`、`memoryScope` | 否 |
| Feature Namespace | MCP / Memory / future middleware 私有配置 | `mcp.serverId` | 否 |

### 4.1 Transport Context

由 HTTP header、网关认证上下文或 A2A SDK server call context 提供。

| 字段 | 说明 | v1 约定 |
|---|---|---|
| `X-Tenant-Id` | 租户标识 | 优先级最高；runtime 不认证，仅传播 |
| Authorization / cookie / gateway claim | 认证信息 | v1 不由 runtime 解释 |

生产部署中，`tenantId` 应由网关或上游平台认证后注入。业务调用方不应把租户身份塞进 `message.metadata`。
...
| `message.metadata` | A2A message 扩展字段 | 不承载 runtime metadata；兼容期只允许 legacy fallback |
...
| 分类 | 字段 | 谁写入 | 谁读取 |
|---|---|---|---|
| Identity | `tenantId`、`userId`、`agentId` | gateway / caller / sample client | `A2aAgentExecutor`、`RuntimeIdentity`、日志、审计 |
...
|---|---|---|---|---|---|---|
| `tenantId` | string | 否 | `X-Tenant-Id` | `X-Tenant-Id` > `params.tenant` > request metadata `tenantId` > `default` | `RuntimeIdentity`、日志、Memory scope、State key | 租户传播，不做认证 |
| `userId` | string | 否 | request metadata `userId` | request metadata > legacy `message.metadata.userId` > `system` | `RuntimeIdentity`、Memory scope、审计 | 终端用户身份 |
| `agentId` | string | 否 | request metadata `agentId` | request metadata > legacy `message.metadata.agentId` > `handler.agentId()` | `RuntimeIdentity`、AgentCard、State key、日志 | 当前目标/逻辑 Agent |
...
| `agentStateKey` | string | 否 | request metadata `agentStateKey` | 显式值 > legacy state key > `state:{tenantId}:{agentId}:{sessionId}` | AgentState、Checkpointer、WaitForUser resume | 短期状态 key |
...
| 字段 | 使用场景 | 不应用于 |
|---|---|---|
| `tenantId` | 多租户隔离、日志、审计、默认 Memory/State key 组成 | 业务消息内容 |
...
兼容期内，runtime 可以从 `message.metadata` 读取 `userId`、`agentId`、`agentStateKey`、`stateKey` 作为 fallback，但必须打 deprecation log，方便样例和外部调用方迁移。
...
1. `X-Tenant-Id` / server call context state
2. A2A `params.tenant`
3. request-level metadata `tenantId`
4. legacy `message.metadata.tenantId`
5. `default`
...
2. legacy `message.metadata.userId`
3. `system`
...
2. legacy `message.metadata.agentId`
3. `handler.agentId()`
...
不再读取 `metadata.sessionId` 作为推荐路径。兼容期内即使读取，也只用于 legacy fallback，并输出 deprecation log。
...
2. request-level metadata `stateKey`，作为旧别名
3. legacy `message.metadata.agentStateKey`
4. legacy `message.metadata.stateKey`
5. `state:{tenantId}:{agentId}:{sessionId}`
...
不建议从 legacy `message.metadata` 读取 `memoryScope`，避免长期记忆 scope 被业务消息污染。
...
private Map<String, Object> resolveLegacyMessageMetadata(RequestContext ctx);
```

行为：

1. `resolveIdentity` 只使用 transport context + request metadata + legacy fallback。
2. `resolveRuntimeMetadata` 不再无条件合并 `message.metadata`。
3. `resolveLegacyMessageMetadata` 只读取兼容字段，并打 deprecation log。
4. `RuntimeMessage` 仍可保留 message 自身 metadata 字段，但 runtime 不再把它当身份来源。
...
- Versatile 样例暂不纳入本轮迁移范围。

### Wave 2：代码兼容读取

`A2aAgentExecutor` 短期保留兼容读取：

1. runtime identity 优先从 request-level metadata 读取。
2. 若缺失，再读取旧的 `message.metadata`。
3. 读取 legacy 字段时打 deprecation log，例如：

```text
[A2A] legacy message metadata key used key=userId taskId=... contextId=...
```

4. `message.metadata.sessionId` 不再作为推荐来源；默认使用 `message.contextId`。

### Wave 3：移除 legacy fallback

当样例和外部调用方迁移完成后：

- 移除 runtime identity 对 `message.metadata` 的读取。
...
| request metadata 缺失，message metadata 提供 `userId` | legacy fallback 生效，并输出 deprecation log |
| `X-Tenant-Id` 与 metadata tenant 冲突 | `X-Tenant-Id` 胜出 |
...
- 是否仍有新增样例把 `userId` / `agentId` / `sessionId` 放进 `message.metadata`？
...
- 是否保留了 `X-Tenant-Id` 的最高优先级？
- 是否对 legacy fallback 打了可观测日志？
...
这样可以减少 OpenJiuwen、AgentScope、Memory、MCP 等能力之间的 metadata 冲突，也为后续多租户权限、工具审计和长期记忆治理留下清晰边界。Versatile adapter 的 metadata 迁移暂不纳入本轮，后续可在同一分层原则下单独设计。
