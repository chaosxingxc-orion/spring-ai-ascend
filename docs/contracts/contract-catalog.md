...
**SPI count by module (shipped surface; the agent-runtime SPI surface is the framework-neutral `engine.spi` set `AgentRuntimeHandler` + `AgentRuntimeProvider` + `StateProvider` + `StreamAdapter`):**
...
| `agent-runtime` | 4 (`AgentRuntimeHandler`, `AgentRuntimeProvider`, `StateProvider`, `StreamAdapter`) |
