...
| L0-GAP-002 | Formal version scope system content. | BA scenarios, feature use cases, function points, harnesses, and delivery slices remain empty or mixed with architecture facts. | Version-scope system removed in the design-month wind-down; gap no longer applies. |
...
| ARCH-UNABSORBED-007 | Roadmap pointers and wave-specific reintroduction triggers. | Important for version/scope planning but not all are architecture facts. | Split into governance/status ledgers (version-scope system removed). |
