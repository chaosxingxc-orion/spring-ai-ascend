#!/usr/bin/env python3
"""Validate orchestrator (sequential lead) team dogfood via run_events."""
...
def validate(events: list[dict[str, Any]], team_expert: str) -> tuple[bool, list[str], list[str]]:
    ...
    if member_starts < 2:
        warnings.append("expected >=2 team.member.started (sequential members)")

    if "team.parallel.started" in names:
        ok = False
        info.append("FAIL: orchestrator should not emit team.parallel.started")
...
def main() -> int:
    if len(sys.argv) != 3:
        print("Usage: dogfood_orchestrator_validate.py <run-events.json> <team-expert-id>", file=sys.stderr)
        return 2
