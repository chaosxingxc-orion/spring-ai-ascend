...
| groupId : artifactId | `com.huawei.ascend : agent-trip` |
| version | `0.1.0` |
...
| 依赖 | `com.openjiuwen:agent-core-java:0.1.12`、`com.huawei.ascend:agent-hotel:0.1.0-SNAPSHOT` |
...
// 下游酒店子智能体（agent-hotel）+ 客户端；两模块 LlmConfig 字段一致，直接映射
try (HotelPlanningAgent hotelAgent = new HotelPlanningAgent(
        new com.huawei.ascend.examples.hotel.LlmConfig(
...
> 提示：`agent-hotel` 的 `HotelPlanningAgent` 实现了 `AutoCloseable`，用 try-with-resources 自动 `close()`（退出时摘掉它注册到全局 `Runner` 的工具）。
...
TripPlanningAgent.chat(userMessage)
  ...
  │                └─ HotelPlanningAgent.chat(nl)  ← 同进程，内部又是一轮 ReAct
...
| 对齐项 | 约定 |
|---|---|
| 集成方式 | 同进程 Java 函数调用：`plan_hotel` → `HotelPlannerClient.plan` → `HotelPlanningAgent.chat(nl)` |
| 出站抽象 | `HotelPlannerClient` 接口；本期实现 `LocalHotelPlannerClient`（委托 agent-hotel），`A2aHotelPlannerClient` 为 A2A 预留（throw） |
...
├── README.cn.md                       ← 本文件
├── pom.xml                            # packaging=jar；依赖 agent-core-java + agent-hotel
├── query.txt                         # 本地调试输入（不入 jar；上游调用不读它）
├── .gitignore                        # 忽略 llm.properties（含真实 Key）
└── src/
    ├── main/
    │   ├── java/com/huawei/ascend/examples/trip/
    │   │   ├── TripPlanningAgent.java          # ★ 入口类：ReActAgent + 唯一工具 plan_hotel
    │   │   ├── LlmConfig.java                  # LLM 配置 record + load()
    │   │   ├── TripSampleMain.java             # main 串调 Sample（已接入真实酒店子智能体）
    │   │   ├── prompt/SystemPromptBuilder.java # 拼 today + 编排规则
    │   │   └── hotel/
    │   │       ├── HotelPlannerClient.java     # 出站抽象
    │   │       ├── LocalHotelPlannerClient.java# 同进程委托 HotelPlanningAgent（agent-hotel）
...
4. **A2A wrapper 合入时机**：等 `agent-runtime` 具备后新增 `examples/travel/agent-trip-a2a`，把出站 `LocalHotelPlannerClient` 换成 `A2aHotelPlannerClient`，本模块代码不动。
